...
import org.df4j.core.connector.reactivestream.ReactiveSubscriber;
 ...
 */
public class AsyncServerSocketChannel0 extends Actor1<ServerConnection>
        implements ReactiveSubscriber<ServerConnection>
        ,CompletionHandler<AsynchronousSocketChannel, ServerConnection>
{
    protected final Logger LOG = Logger.getLogger(AsyncServerSocketChannel0.class.getName());
    /**
    ...
    public AsyncServerSocketChannel0(SocketAddress addr, int connCount) throws IOException {
        if (addr == null) {
     ...
     */
    @Action
    protected void act(ServerConnection serverConn) {
        if (serverConn==null) {
            onCompleted();
        } else {
            try {
                assc.accept(serverConn, this);
            } catch (Exception e) {
                close();
            }
        }
