...
import com.github.rfq.dffw.core.*;


/**
 * A set of identical Ping Actors, passing tokens to a single Pong actor,
 * which returns tokens back to sender.
 * A tokens dies after predefined number of hops.
 *
 */
public class PingPongTest {
    final static int NUM_ACTORS = 100; // number of nodes
    final static int NUM_TOKENS = NUM_ACTORS*10; // number of tokens
    final static int TIME_TO_LIVE = 10000; // hops 
    ...
    public void test1() throws InterruptedException {
		String workerName = Worker.class.getCanonicalName();
    ...
    static class Ping extends Actor<Token> {
         ...
         * For each received packet, decrease the number of remaining hops.
         * If number of hops become zero, send it to sink,
         * otherwise send to the Pong actor.
         */
        @Override
    ...
    static class Pong extends Actor<Token> {
        ...
        /** the method to handle incoming messages
         * for each received packet, decrease the number of remaining hops.
         * If number of hops become zero, send it to sink,
         * otherwise send it back to the Ping actor.
         */
        @Override
        protected void act(Token token) throws Exception {
            int nextVal = token.hops_remained - 1;
    ...
    /**
     * The ending node. This is a Port rather than actor, to be accessed outside the actor world.
     */
    ...
    float runPingPong(Executor executor) throws InterruptedException {
        ...
        // actors can only be accessed from threads which carry executor as a thread local variable,
