...
//            final PGroup group = new DefaultPGroup(new DefaultPool(true, concurrencyLevel));
            ...
            Actor1<String> lastActor = null;

            // Create and chain actors (backwards - the lastActor will be first)
            for (int i = 0; i < ACTORS; i++) {
                final Actor1<String> actor = new MessagePassingActor(lastActor, cdl);
...
//            group.shutdown();
     ...
     */
    public static class MessagePassingActor extends Actor1<String> {

        private final Actor1<String> soFarLast;
        private final CountDownLatch cdl;

        MessagePassingActor(final Actor1<String> soFarLast, final CountDownLatch cdl) {
