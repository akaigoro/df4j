 ...
 *
 *  The actor realm in df4j is located at the package {@link org.df4j.core.dataflow}. The base node class is {@link org.df4j.core.dataflow.Actor}.
 *  It extends {@link org.df4j.core.dataflow.AsyncProc} and so has the default result and can contain one-shot parameters (which can be set only once).
 ...
 *   The node classes differ only that after calling the action procedure, the method {@link org.df4j.core.dataflow.Actor#start} is called again.
 *
 *   An interesting case is calling {@link org.df4j.core.dataflow.Actor#start} in an asynchronous callback like in
