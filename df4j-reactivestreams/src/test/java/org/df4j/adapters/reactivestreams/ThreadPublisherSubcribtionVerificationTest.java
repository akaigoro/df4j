...
import org.reactivestreams.example.unicast.ThreadPublisherSubscribtion;
...
public class ThreadPublisherSubcribtionVerificationTest extends PublisherVerification<Long> {
    static final long defaultTimeout = 400;

    public ThreadPublisherSubcribtionVerificationTest() {
        super(new TestEnvironment(defaultTimeout));
    }

    @Override
    public Publisher<Long> createPublisher(long elements) {
        ThreadPublisherSubscribtion publisher = new ThreadPublisherSubscribtion(elements);
        publisher.start();
        return publisher;
    }
